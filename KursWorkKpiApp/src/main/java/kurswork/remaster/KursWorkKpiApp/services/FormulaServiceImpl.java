...
//		System.out.println("Ball : " + expression1.calculate());
		...
		List<Variable> variables = formula.getFormulaVariables().stream()
				.map(criteiaVariable->criteiaVariable.getVariable()).sorted((e1,e2)->e1.getVariable_id() - e2.getVariable_id())
				.collect(Collectors.toList());
		variables.stream().forEach(var->System.out.println("\t" + var.getVariable_sign() + "\n\t\t" + var.getVariable_descr()));
		List<List<InsertedVariable>> insertedVariablesOfEmployye = variables.stream()
				.map(var->var.getInsertedVariables().stream()
						...
						.sorted((e1,e2)->e1.getInsVar_id()-e2.getInsVar_id())
						.collect(Collectors.toList()))
				.collect(Collectors.toList());
		...
		if (formulaStr.contains("sum")) {
			List<List<InsertedVariable>> multipleVariables = insertedVariablesOfEmployye.stream()
					.filter(list->!list.get(0).getVariable().getCount().equals("1"))
					.collect(Collectors.toList());
			List<List<InsertedVariable>> singleVariables = insertedVariablesOfEmployye.stream()
					.filter(list->list.get(0).getVariable().getCount().equals("1"))
					.collect(Collectors.toList());;
			...
			for (int i = 0; i < multipleVariables.size(); i++) {
				...
				InsertedVariable insertedVarForStep = singleVariables.stream()
						.flatMap(varList->varList.stream())
				...
				formulaStr = formulaStr.replaceAll(multipleVariables.get(i).get(0).getVariable().getVariable_sign(), "par(i + ("+ step +" * "+i+"))");
			}
			formulaStr = formulaStr.replaceAll("n", "[npar]/" + multipleVariables.size());
			
			List<Double> params = new ArrayList<>();
			multipleVariables.stream().forEach(valuesList->{
				params.addAll(valuesList.stream()
						.map(valueObj->Double.parseDouble(valueObj.getInserted_value()))
						.collect(Collectors.toList()));
			});
			...
			List<Double> params = insertedVariablesOfEmployye.stream()
